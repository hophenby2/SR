...
RenderFunc()
RenderFunc()
check(old_render_count == 0, "visible lockstep ignored the display sampling interval")
instance.frame = instance.frame + 1
RenderFunc()
RenderFunc()
check(old_render_count == 1, "visible lockstep did not render a sampled logical frame exactly once")
...
RenderFunc()
check(old_render_count == 2, "rendering did not resume after disconnect")
